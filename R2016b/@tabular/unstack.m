...
%   U = UNSTACK(S,DATAVAR,INDVAR) converts the table S to an equivalent
%   table U that is in "unstacked format", by "unstacking" a single variable in
%   S into multiple variables in U. In general U contains more
%   variables, but fewer rows, than S.
%
%   DATAVAR specifies the data variable in S to unstack.  INDVAR specifies an
%   indicator variable in S that determines which variable in U each value
%   in DATAVAR is unstacked into, as described below.  UNSTACK treats the
%   remaining variables in S as grouping variables.  Each unique combination
%   of their values defines a group of rows in S that will be unstacked into
%   a single row in U.
%
...
elseif all(strcmp(rowLabelsName,groupVars))
    % Group by the row labels, with no other grouping variables.
    groupVars = 0;
else
    % Convert groupVars to indices.  [] is valid, and does not indicate "default".
    try
        groupVars =  a.varDim.subs2inds(groupVars); % a row vector
    catch ME
        subs2indsErrorHandler(a,groupVars,ME,'unstack:MixingDataAndDimGroupingVars',rowLabelsName{:}); %    
    end
...
if supplied.ConstantVariables
    if all(strcmp(rowLabelsName,constVars))
        constVars = 0;
    else
        try
            constVars = a.varDim.subs2inds(constVars); % a row vector
        catch ME
            subs2indsErrorHandler(a,constVars,ME,'unstack:MixingDataAndDimConstVars',rowLabelsName{:});
        end
    end
    if ~supplied.GroupingVariables
    ...
    % If the specified constant vars is the input's row labels, i.e. constVars==0,
    % those will automatically be present in the output, no constant vars need be
    % copied over, so clear constVars.
    if isequal(constVars,0), constVars = []; end
end
...
if useDfltWideVarNames
    wideVarNames = matlab.internal.tableUtils.makeValidName(dfltWideVarNames(:)','varnamesWarn'); % allow mods, these are never empty
end

% Create the wide table from the unique grouping var combinations. This carries
% over properties of the tall table. If grouping by the input's row labels, i.e.
% groupVars==0, the "unique grouping var combinations" will automatically be
% present in the output's row labels, no grouping vars need be copied over, so
% clear groupVars after getting the group indices.
[jdx,~,idx] = table2gidx(a,groupVars);
if isequal(groupVars,0), groupVars = []; end
...
for t = 1:ntallVars
    ...
    if isnumeric(tallVar) || islogical(tallVar) % but not char
        ...
        for k = 1:ncols
            ...
            % Fill values separately (more efficient)
            fillInLocs = ~logical(accumarray({jdx,kdx},1,[nrowsWide,nwideVars]));
            wideVars_k(fillInLocs) = fillVal;
    ...
    % If wide var names were given by NewDataVariableNames, duplicate names are an error,
    % caught by setLabels here or by checkAgainstVarLabels below.
    try
