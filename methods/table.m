classdef (Sealed) table < matlab.mixin.internal.indexing.DotParen
%TABLE Table.
...
%   Copyright 2012-2015 The MathWorks, Inc.
    ...
    properties(Constant, GetAccess='private')
        ...
        propsDflts = struct('Description'         , {''},...
                            ...
                            'UserData'            , []);
        propertyNames = [ fieldnames(table.propsDflts); {'RowNames'; 'VariableNames'} ];
    end
...
%   See also READTABLE, CELL2TABLE, ARRAY2TABLE, STRUCT2TABLE. 
    ...
    methods(Hidden = true)
        ...
        function p = properties(t)
            ...
            if nargout == 0
                fprintf('%s\n',getString(message('MATLAB:ClassUstring:PROPERTIES_FUNCTION_LABEL','table')));
                fprintf('    %s\n',pp{:});
            else
                p = pp;
            end
        ...
        function vars = getVars(t)
            ...
            vars = cell2struct(t.data,t.varnames,2);
        end
        ...
        function props = getProperties(t)
            ...
            props = t.props;
            props.RowNames = t.rownames;
            props.VariableNames = t.varnames;
        end
        ...
        function sz = numArgumentsFromSubscript(t,s,context)
            ...
            elseif strcmp(s(end).type,'()')
                ...
                if strcmp(s(1).type,'{}')
                    x = subsrefBraces(t,s(1));
                elseif strcmp(s(1).type,'.')
                    x = subsrefDot(t,s(1));
                    if ~isequal(s(2).type,'.') % t.Var(...) or t.Var{...}
                        % Dot-parens or dot-braces subscripting might use row names inherited from the
                        ...
                        if isnumeric(rowIndices) || islogical(rowIndices) || iscolon(rowIndices)
                            ...
                            % getRowIndices returns the indices as a col vector, but subscripting on a table
                ...
                sz = matlab.internal.table.numArgumentsFromSubscriptRecurser(x,s(2:end),context);
            end
        end
        ...
        function d = double(d), error(message('MATLAB:table:InvalidNumericConversion','double')); end %#ok<MANU>
        function d = single(d), error(message('MATLAB:table:InvalidNumericConversion','single')); end %#ok<MANU>
