...
if (t.nrows > 0) && (t.nvars > 0)
    ...
    dispVarNames();
    rowpos = unique([0 CHUNK_SIZE:CHUNK_SIZE:t.nrows, t.nrows]);
    for r = 2:numel(rowpos)
        pos = rowpos(r-1)+1:rowpos(r);
        for ivar = 1:t.nvars
            ...
            if ivar == 1 % starting over at left margin
                tblCharsChunk = [tblChars(pos,:) varChars];
            else
                tblCharsChunk = [tblChars(pos,:) betweenSpaces(pos,:) varChars];
            end
        end
        disp(tblCharsChunk);
    end
