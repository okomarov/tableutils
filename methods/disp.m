...
if (t.nrows > 0) && (t.nvars > 0)
    ...
    rowpos = unique([1 CHUNK_SIZE:CHUNK_SIZE:t.nrows, t.nrows+1]);
    for r = 1:numel(rowpos)-1
        pos                         = rowpos(r):rowpos(r+1)-1;
        [tblCharChunk, varnamePads] = getTableChar(t, tblChars, pos);
        if r == 1
...
%-----------------------------------------------------------------------
    function [tblCharsChunk, varnamePads] = getTableChar(t,tblChars,pos)
    ...
    for ivar = 1:t.nvars
            ...
            if ~isempty(var) && ismatrix(var) && (size(var,2) <= 3)
                ...
                elseif iscell(var)
                    ...
                    if length(loc) == n+1+isLoose % can use them as row delimiters
                        ...
                        % same length.
                        varChars = char(zeros(size(var,1),0));
                        offset   = 0;
                        for j = 1:m
                            if j > 1
        ...
        if ivar == 1 % starting over at left margin
            ...
            tblCharsChunk = [tblCharsChunk betweenSpaces(pos,:) varChars]; %#ok<AGROW>
        end
    end
    end
